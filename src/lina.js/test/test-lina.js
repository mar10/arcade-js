...
test("Point2", function() {
	...
	assertEqual(""+p1, "(10/11)", "pt.toString");

	p2 = new Point2(p1);
	ok(p2.x == 10 && p2.y == 11, "pt.copy constructor");

	p1.translate(3, 7);
	assertEqual(p1, {x:13, y:18}, "pt.translate");
	assertEqual(p2, {x:10, y:11}, "pt.copy constructor is a real copy");

	v1 = new Vec2(11, 13);
	p1.translate(v1);
	assertEqual(p1, {x:24, y:31}, "pt.translate by vector");

	assertEqual(new Point2(1,2).rotate(0.5*Math.PI),
			{x:-2, y:1}, "pt.rotate by 90°");
	
	p1 = new Point2(1, 2);
	p2 = Point2.rotate(p1, 0.5*Math.PI);
	assertEqual(p2,
